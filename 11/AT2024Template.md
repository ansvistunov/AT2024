...
1. Требования внешних интерфейсов
   - Интерфейс оператора системы. Система предоставляет:
      ...
      5. Таблицу карт состояний работников и закрепленные за ними устройства диагностики, содержащие:
         1. Индификатор устройства.
         2. Индификатор работника.
    ...
    - Интерфейс системного администратора. Предоставляет все необходимые инструменты для отладки системы и её диагностики:
      1. Интерфейс личного кабинета администрарора системы, позволяющий просматривать личные кабинеты других пользоывателей системы.
      ...
      3. Интерфейс режима технического обслуживания, содержащий:
         1. Режим отладки утройств диагностики.
         2. Режим отладки дополнительной аппаратуры.
         3. Режим работы с учетными записями пользователей системы (операторов).
    
    - Интерфейс работника. Предоставляет интерфейс взаимодействия работника с устройством диагностики и определяется используемой моделью устройства диагностики.
    
    - Аппаратные интерфейсы. Система поддерживает: 
        - Устройства диагностики, внесенные в реестр используемых на территории предприятия, в частности активные в момент  работы системы. 
    ...
    - Программные интерфейсы. Система использует:  
        ...
        - СУБД Oracle/PostgreSQL для организации БД учета используемых УД, получаемых показаний с УД, ответов АК и журналов событий.
        - Система реализована на базе операционной системы X.

    - Интерфейсы коммуникаций: 
      - Прием данных, отправляемых с устойств диагностики, реализован по технологии Wi-Fi, поддерживающей локальную сеть на территории предприятия. 
...
2. Функциональные требования
    1. Функции для Работника
        1. Система должна принимать и обрабатывать аварийный сигнал от устройства диагностики (УД), когда работник нажимает аварийную кнопку, и немедленно передавать его оператору.
        2. Система должна информировать работника, если устройство диагностики неисправно или потеряна связь с системой.
    2. Функции для Оператора
        1. Система должна уведомлять оператора в случае потери соединения с устройством диагностики (УД) конкретного работника.
        2. Система должна уведомлять оператора в случае потери соединения с аналитической компанией (АК).
        3. Система должна предоставлять возможность видеть статус соединения с устройствами диагностики (УД) в режиме реального времени.
        4. Система должна предоставлять возможность видеть статус соединения с аналитической компанией в режиме реального времени.
        5. Система должна уведомлять оператора в случае невозможности передачи данных в АК после трех попыток.
        6. Система должна предоставлять возможность вручную проверять состояние соединения с устройствами диагностики и аналитической компанией.
        7. Система должна отображать результаты анализа состояния здоровья работников в виде цветовых индикаторов:
            - Зеленый (OK): состояние в норме
            - Желтый (WARNING): состояние требует внимания
            - Красный (CRITICAL): критическое состояние.
        8. Система должна предоставлять доступ к журналу событий, где регистрируются любые изменения состояния здоровья работников, передача данных и аварийные сигналы.
        9. Система должна предоставлять возможность просматривать полную историю состояний каждого работника, включая информацию о предыдущих аварийных сигналах.
        10. Система должна предоставлять информацию о состоянии здоровья каждого работника на интерфейсе в режиме реального времени.
        11. Система должна предоставлять возможность вручную проверять состояние соединения с устройствами диагностики и аналитической компанией
        12. Система должна отправлять звуковое и визуальное уведомление оператору при получении аварийного сигнала от любого работника.
        13. Система должна реагировать на аварийный сигнал независимо от наличия данных ЭКГ, в том числе при временной потере данных.
    3. Функции для Системного администратора
        1. Система должна предоставлять возможность настраивать и управлять базой данных для правильного хранения и обработки данных.
        2. Система должна предоставлять возможность просматривать и анализировать журналы событий (логи), включая информацию о передаче данных, ошибках и сбоях.
        3. Система должна предоставлять возможность добавлять новые устройства диагностики (УД) и удалять старые или неисправные устройства.
        4. Система должна предоставлять возможность обновлять программное обеспечение системы и устройств диагностики.
        5. Система должна предоставлять возможность проводить резервное копирование базы данных.
        6. Система должна предоставлять возможность восстанавливать систему после сбоев с использованием резервных копий данных.
        7. Система должна предоставлять возможность управлять учетными записями пользователей, создавая новые профили и удаляя устаревшие.

3. Требования исполнения
    1. Отслеживание состояния здоровья работников
        1. Система должна отправлять уведомление оператору не позднее, чем через 5 секунд после потери соединения с устройством диагностики (УД).
        2. Система должна обеспечивать обработку данных от не менее чем 100 УД, работающих одновременно на территории предприятия, без снижения производительности.
    2. Аварийное оповещение
        1. Система должна отправить аварийный сигнал оператору в течение 2 секунд после нажатия аварийной кнопки на устройстве диагностики (УД).
    3. Передача данных ЭКГ
        1. Система должна обрабатывать данные ЭКГ, поступающие с каждого устройства диагностики (УД), каждые 10 секунд.
        2. Система должна передавать собранные данные ЭКГ в аналитическую компанию (АК) каждые 30 секунд.
    4. Получение результатов анализа
        1. Система должна получать результаты анализа ЭКГ от АК не позднее чем через 10 секунд с момента отправки данных.
    5. Передача результатов анализа
        1. Обновление цветовых индикаторов на экране системы должно происходить не позднее 1 секунды после получения результата анализа.
        2. Система должна предоставлять возможность оператору просматривать историю состояний каждого работника за последние 3 месяца.
...
| ------------------------ | ------------------- | ------------------------------ | -------------------------------------------------------------------------- | ------------------------------------------- | -------------------------------------------------------------------------- |
| Идентификатор устройства | integer             | Primary key                    |                                                                            | Достумен оператору системы и администратору |                                                                            |
| Состояние устройства     | boolean             |                                | Принимает значения 0 или 1                                                 | Достумен всем пользователям                 |                                                                            |
| Данные ЭКГ               | integer             |                                | Соответствует ограничениям, указанным в договоре о предоставлении услуг АК | Достумен оператору системы и администратору | Соответствует ограничениям, указанным в договоре о предоставлении услуг АК |

- **Таблица учета состояний работников**

| Атрибут записи                       | Тип хранимых данных | Сущности данных и их отношения | Ограничения целостности                                                    | Способы доступа                             | Требования хранения данных                                                |
| ------------------------------------ | ------------------- | ------------------------------ | -------------------------------------------------------------------------- | ------------------------------------------- | ------------------------------------------------------------------------- |
| Идентификатор записи                 | integer             | Primary key                    |                                                                            | Достумен оператору системы и администратору | Прошел инструктаж по использованию УД и проц                              |
| Идентификатор устройства             | integer             | Foreign key                    |                                                                            | Достумен оператору системы и администратору | Прошел обучение по экстренному реагированию                               |
| Идентификатор работника              | integer             | Foreign key                    |                                                                            | Достумен оператору системы и администратору | Прошел обучение по экстренному реагированию                               |
| Данные ЭКГ                           | integer             | Foreign data from Table №2     | Соответствует ограничениям, указанным в договоре о предоставлении услуг АК | Достумен оператору системы и администратору | Соответствует требованиям, указанным в договоре о предоставлении услуг АК |
| Результаты от АК                     | custom              |                                | Соответствует ограничениям, указанным в договоре о предоставлении услуг АК | Достумен оператору системы и администратору | Соответствует требованиям, указанным в договоре о предоставлении услуг АК |
| Индификатор нажатия аварийной кнопки | boolean             |                                |                                                                            | Достумен оператору системы и администратору |                                                                           |

5. Характеристики программного обеспечения системы
