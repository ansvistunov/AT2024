...
|ID1|Работа со справочником зон с ограниченим скорости||| ПЕРЕ ПЕРЕ ПЕРЕ ПЕРЕ ПЕРЕ ПЕРЕМЕСТИТЬ {{{{{{{{{{{[[[[[[[[[{{{{{{{{[[[

**Работа с техником по обслуживанию**
|Идентификатор|Наименование	|Содержание|	Важность|
|-------------|-----------------|----------|------------|
|ID12| Запрос информации	|Отправка запроса на сервер на получение информации о самокате |Важно|
...
**Cправочники**
 - Справочник пользователей - справочник, в котором хранятся данные о пользователях, включая их ФИО, данные для их авторизации в системе. Пользователи приложения для аренды автомобилей, прошедшие регистрацию в офисе компании, получают отметку в справочнике, дающую им доступ к приложению; пользователи без этой отметки, например, пользователи приложения для аренды самокатов, не проходившие регистрацию в офисе, не могут авторизоваться в приложении дял аренды автомобилей.
 - Справочник транспортных средств - справочник, в котором хранится информация о транспортных средствах, включая их местоположение и техническое состояние.
 - Справочник зон парковки - справочник, в котором хранится информация о зонах, где разрешена парковка ТС.
 - Справочник зон с ограничениями скорости - справочник, хранящий зоны, при движении в которых на автомобиле свыше разрешённой скорости на пользователя накладываются штрафы, и значения максимальной разрешённой скорости для каждой из этих зон.
