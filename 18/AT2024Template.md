...
|Идентификатор	|Наименование	|Содержание|	Важность	|Стабильность|
|------------|--------|------------------|---------------|--------------|
|ID-0.10	|Создание списка ТО|	Создание списка ТО и ведение его учета. Автоматическое оповещение о каких-либо неполадках. |	Высокая|	Средняя. Простой вероятен не более 10 минут в неделю.|
...
|Идентификатор|	Наименование	|Содержание|	Важность|	Стабильность|
|------------|--------|------------------|---------------|--------------|
|ID-16|Оптимизация графиков ремонта производственных цепочек|Система имеет возможность оптимизации графиков запланированного и незапланированного ремонта, учитывая связи между оборудованием в производственных узлах, которые потом объединяются в цепочки. В случае неисправности одного элемента узла, система предлагает провести ремонт всего узла на каком-то уровне в цепочке для максимального снижения простоев оборудования. Причем ремонт данного неисправного узла встанет в первый высший приоритет, из-за вероятности остановки всего узла.|Высокая	|Критическая. Простой максимум 1 рабочий день в полгода.|
|ID-17|Приоритетность ремонтных работ|Система имеет возможность установки приоритетов для различных видов ремонтных работ (по времени, наработке и внеплановые инспекции). Срочность ремонта (например, «немедленное решение», «в течение смены», «плановое»). Влияние неисправного оборудования на производство (например, «остановка производства», «снижение производительности»). Это позволяет определить, какие работы следует выполнять в первую очередь при составлении графиков ремонта. Причем сам инспектор может внести коррективы в приоритетность работ.|Высокая	|Критическая. Простой максимум 1 рабочий день в полгода.|
|ID-18|Интеграция в узлы и с другими узлами|Система имеет возможность интеграции с другими узлами, такими которые взаимосвязаны, и работают в единой системе, которые после образуют цепочку. Интеграция позволяет автоматически обновлять информацию о доступных материалах и запчастях, а также синхронизировать данные о выполненных регламентных работах с другими системами.|Высокая	|Средняя. Простой максимум 1 неделя в полгода.|
|ID-19|История изменений|Система сохраняет историю изменений в графике обслуживания и выполненных работах. Пользователи имеют возможность просмотра предыдущих версий графика и отчетов, а также видят информацию о том, кто и когда внес изменения. История изменений поможет отслеживать и анализировать выполненные работы.|Средняя	|Средняя. Простой максимум 1 неделя в полгода.|
|ID-20|Права и роли|Система поддерживает различные пользовательские роли и права доступа. Администратор определяет роли пользователей и назначает им соответствующие права доступа. Администратор имеет полный доступ ко всем функциям системы, в то время как обычные пользователи имеют ограниченные права. Особое право имеет Инспектор, который имеет частично права администратора, а частично пользователя.|Высокая	|Средняя. Простой максимум 1 день в полгода.|
...
Все ремонтные требования подлежат выполнению в виде данного шаблона: 
*Идентификатор* (Однозначно идентифицирует требование во всех документах), *Наименование* (Определяет требовательную группу, к которой относится требование), *Содержание* (Описывает сами требования), *Важность* (Степень важности выполнения требования: 1 - "Высокая", 2 - "Средняя", 3 - "Низкая"), *Стабильность* (Влияние стабильности работы на проект (если вдруг сломается - то сильно или не сильно повлияет на весь процесс работы): 1 - Критическая (сильно зависит), 2 - Средняя, 3 - Малая.).
Данные о важности и стабильности могут варьироваться в зависимости от конкретного проекта. 

|Идентификатор|	Наименование	|Содержание|	Важность|	Стабильность|
|------------|--------|------------------|---------------|--------------|
|AB-1|Стандарты кода и написание|В соответствии с тем, на каком языке программирования пишется система СКОТ, следуйте установленым требованиям, прописанным в документации того языка, который вы используете. Проверьте, читабелен ли ваш код, понятен ли будет тем специалистам, которым вероятно прийдется с ним разбираться, для устранения каких-либо проблем или для решения иных вопросов. Оставление комментариев - приемлимо. Просим также прописывать в начале файла описание какая переменная за что отвечает и ввести единую систему имен переменных.|1|1|
|AB-2|Использование библиотек и фреймворков|Используйте надежные и проверенные библиотеки и фреймворки, чтобы упростить сопровождение и обеспечить стабильность системы.|1|1|
|AB-3|Разделение на модули и компоненты|Разбейте систему на модули и компоненты, что облегчить отслеживание и обслуживание различных частей системы.|1|1|
|AB-4|Резервное копирование и восстановление|Предусмотрите механизмы автоматического резервного копирования данных, место, куда будет вся нужная информация копироваться, и возможность их восстановления в случае сбоев.|1|1|
|AB-5|Тестирование и контроль версий|Разработайте набор тестов, включая модульные, интеграционные и функциональные тесты, для обнаружения и исправления ошибок. Используйте систему контроля версий, такую как Git, для отслеживания изменений в коде и управления версиями.|1|1|
